...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 會員等級過濾
    if (membershipLevel) {
      query = query.eq("membershipLevel", membershipLevel);
    }
    ...
    if (error) {
      logger.error("Error fetching users", error, { requestId });
    ...
    logger.error(
      ...
      { endpoint: "/api/admin/users", requestId }
    );
    return createErrorResponse(
      error instanceof Error ? error : new Error("獲取會員列表失敗"),
      requestId
    );
  }
}
